...
export function Tokyo(props) {
  const group = useRef()
  const busRef = useRef() // Reference for the bus
  ...
  useFrame((state, delta) => {
    setBusPosition((prev) => {
      // Left-most part of the road should be treated as the "start"
      const roadEnd = -80   // left-most bound (start)
      const roadStart = 80  // right-most bound (end)
      const newPosition = prev + delta * 45 * direction

      // Reverse direction when reaching the bounds
      if (newPosition <= roadEnd) {
        setDirection(1); // Start heading right
      } else if (newPosition >= roadStart) {
        setDirection(-1); // Start heading left
      }
      return newPosition
    })

    if (busRef.current) {
      busRef.current.position.x = busPosition
    }
  })
          ...
          <group name="4cd116fc63ca40809810ca0842dc78edfbx" rotation={[Math.PI / 2, 0, 0]}>
            <group name="Object_2">
              <group name="RootNode">
                <group name="Object_4">
                  <group
                    ref={busRef} // Attach reference to Object675
                    name="Object675"
